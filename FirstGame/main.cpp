...
void InitBullets(Bullet bullet[], int size);
...
void CollideBullet(Bullet bullets[], int sizeB, Comet comets[], int sizeC, SpaceShip &ship);

void InitComet(Comet comets[], int size);
void DrawComet(Comet comets[], int size);
void StartComet(Comet comets[], int size);
void UpdateComet(Comet comets[], int size);
void CollideComet(Comet comets[], int sizeC, SpaceShip &ship);
...
int main(void) {
	...
	font16 = al_load_font("opensans.ttf", 16, 0);
	...
	while (!done)
	{
		...
		if (redraw && al_is_event_queue_empty(event_queue))
		{
			...
			if (!isGameOver)
			{
				...
				al_draw_textf(font16, al_map_rgb(255, 255, 255), 5, 5, 0, "Player has %i lives left.  Score: %i", ship.lives, ship.score);
			}
			else
			{
				al_draw_textf(font16, al_map_rgb(0, 255, 255), WIDTH / 2, HEIGHT / 2, ALLEGRO_ALIGN_CENTER, "Game Over.  Final Score: %i", ship.score);
			}
...
void DrawComet(Comet comets[], int size)
{
	for (int i = 0; i < size; i++)
	{
		if (comets[i].live)
		{
			al_draw_filled_circle(comets[i].x, comets[i].y, 20, al_map_rgb(0, 255, 0));
		}
	}
}
void StartComet(Comet comets[], int size)
